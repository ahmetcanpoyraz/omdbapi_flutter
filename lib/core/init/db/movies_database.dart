...
import '../../models/movie_db_model.dart';
...
CREATE TABLE $tableMovies ( 
  ...
  ${MovieFields.type} $textType,
  ${MovieFields.imdbID} $textType,
...
''');
  ...
  Future<Moviedb> readNote(int id) async {
    final db = await instance.database;
    ...
    if (maps.isNotEmpty) {
      return Moviedb.fromJson(maps.first);
    } else {
      throw Exception('ID $id not found');
    }
  }

  Future<List<Moviedb>> readAllMovies() async {
    final db = await instance.database;
  ...
  Future<int> update(Moviedb movie) async {
    final db = await instance.database;
